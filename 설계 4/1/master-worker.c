...
void *generate_requests_loop(void *data)
{
	int thread_id = *((int *)data);
  	...
  	while(1) {
		...
		//wait (only loop) while worker use buffer
...
void *generate_working_loop(void *data)
{
	int thread_id = *((int *)data);
	int item_to_consume;
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < num_workers; i++)
		pthread_create(&worker_thread[i], NULL, generate_working_loop, (void *)&worker_thread_id[i]);
